...
async function referralDrips(conn, referralAddress) {
	const cursor = await r.table('payouts').orderBy({index: r.desc('timestamp')})
		.filter({referralAddress}).limit(config.displayPendingNum).run(conn);
	return cursor.toArray(); // rows
}

module.exports.referralDrips = referralDrips;
